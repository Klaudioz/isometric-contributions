...
const isoJsCode = `
  ...
  let yearTotal = 0, weekTotal = 0, averageCount = 0, maxCount = 0, countTotal = 0, weekCountTotal = 0;
  let streakLongest = 0, streakCurrent = 0;
  let bestDay = null, firstDay = null, lastDay = null, datesTotal = null, weekStartDay = null;
  let weekDatesTotal = null, datesLongest = null, datesCurrent = null, dateBest = null;
  let toggleSetting = 'cubes';

  const resetValues = () => {
    yearTotal = 0; averageCount = 0; maxCount = 0; streakLongest = 0; streakCurrent = 0; weekTotal = 0;
    bestDay = null; firstDay = null; lastDay = null; datesLongest = null; datesCurrent = null; weekStartDay = null;
  }
  ...
  const initUI = () => {
    ...
    if (toggleSetting === 'squares') squaresButton.classList.add('selected');
    ...
    if (toggleSetting === 'cubes') cubesButton.classList.add('selected');

    buttonGroup.append(squaresButton, cubesButton);
  ...
  const getSquareColor = (rect) => rgbToHex(getComputedStyle(rect).getPropertyValue('fill'));
...
// We also need the obelisk.js library. You should download it and place it in your repo.
...
async function generateImage() {
  ...
  // Inject the lodash library from a CDN
  await page.addScriptTag({ url: 'https://cdn.jsdelivr.net/npm/lodash@4.17.21/lodash.min.js' });
  // Inject the obelisk library first, as iso.js depends on it
